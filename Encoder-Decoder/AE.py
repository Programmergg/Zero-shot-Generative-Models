'''
Now its attrs_best performance is unseen=48.77,seen=56.92,h=52.53
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        recon_visual = autoencoder.encoder(input_attv, noisev)
        ...
        recon_att = autoencoder(input_attv, noisev)
    ...
    syn_unseen_feature, syn_unseen_label = generate_syn_feature(autoencoder.encoder, data.unseenclasses, data.attribute, opt.syn_num)
    ...
    if opt.gzsl == True:
        ...
        if cls.H > best_H:
            best_H = cls.H
            torch.save(autoencoder.state_dict(),'../saved_models/seen{0}_unseen{1}_H{2}.pkl'.format(cls.acc_seen, cls.acc_unseen, cls.H))
            print('attrs_best models saved!')
    else:
        syn_feature, syn_label = generate_syn_feature(autoencoder.encoder, data.unseenclasses, data.attribute, opt.syn_num)
