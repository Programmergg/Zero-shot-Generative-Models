'''
Now its attrs_best performance is unseen=48.10,seen=57.89,h=52.54
encoder and decoder separately optimize
'''
import argparse
import random
import torch
...
# setup optimizer
optimizer_encoder = optim.Adam(encoder.parameters(), lr=opt.lr,betas=(opt.beta1, 0.999))
optimizer_decoder = optim.Adam(decoder.parameters(), lr=opt.lr,betas=(opt.beta1, 0.999))
...
def generate_syn_feature(Encoder, classes, attribute, num):
    ...
    with torch.no_grad():
        for i in range(nclass):
            ...
            output = Encoder(syn_att, syn_noise)
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        recon_att = decoder(input_resv)
        recon_att_loss = torch.sum((recon_att - input_attv) ** 2)

        decoder_loss = recon_att_loss
        ...
        encoder.zero_grad()
        recon_visual = encoder(input_attv, noisev)
        ...
        encoder_loss = recon_visual_loss + opt.cls_weight * c_errG_fake + reg_loss
        encoder_loss.backward()
        optimizer_encoder.step()
    print('[%d/%d] visual_loss: %.4f, att_loss: %.4f, total_loss: %.4f' % (epoch, opt.nepoch,recon_att_loss.item(),recon_visual_loss.item(),encoder_loss.item()))

    encoder.eval()
    decoder.eval()
    # Generalized zero-shot learning
    syn_unseen_feature, syn_unseen_label = generate_syn_feature(encoder, data.unseenclasses, data.attribute, opt.syn_num)
    ...
    if opt.gzsl == True:
        ...
        if cls.H > best_H:
            best_H = cls.H
            torch.save(encoder.state_dict(),'../saved_models/seen{0}_unseen{1}_H{2}.pkl'.format(cls.acc_seen, cls.acc_unseen, cls.H))
            print('attrs_best models saved!')
    else:
        syn_feature, syn_label = generate_syn_feature(encoder, data.unseenclasses, data.attribute, opt.syn_num)
