'''
Now its attrs_best performance is unseen=58.06,seen=70.34,h=63.61
InfoGAN
From the paper:
...
import torch
import util
from models import MLP_G_InfoGAN,MLP_D,Auxiliary
import classifier
...
parser.add_argument('--disc_dim', type=int, default=85, help='size of the disc vector')
parser.add_argument('--cont_dim', type=int, default=85, help='size of the cont vector')
parser.add_argument('--z_dim', type=int, default=85, help='size of the z vector')
...
def compute_noise(opt, noise, size, c=None):
    noise.normal_(0, 1)
    disc_c = torch.zeros((size, opt.disc_dim)).cuda()
    if c is not None:
        categorical = int(c) * torch.ones((size,), dtype=torch.long)
    else:
        categorical = torch.randint(0, opt.disc_dim, (size,), dtype=torch.long)
    disc_c[range(size),categorical] = 1

    #Gaussian continuous latent variables
    cont_c = torch.randn(size, opt.disc_dim).cuda()
    res = torch.cat((noise, disc_c, cont_c), dim=1)
    return res
...
def generate_syn_feature(netG, classes, attribute, num):
    ...
    with torch.no_grad():
        for i in range(nclass):
            ...
            final_syn_noise = compute_noise(opt, syn_noise, num)
            output = netG(syn_att, final_syn_noise)
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        for iter_d in range(opt.critic_iter):
            ...
            # train with fakeG
            final_noise = compute_noise(opt, noise, opt.batch_size)
            noisev = Variable(final_noise)
            ...
            # # Approximate true posterier for categorical, Gaussian latent variables
            # Q_discrete, Q_continuous = netQ(fake)
            #
            # # Compute mutual information loss
            # # Discrete component
            # discrete_target = final_noise[:, opt.z_dim:opt.z_dim + opt.disc_dim]
            # disc_loss = F.cross_entropy(Q_discrete, torch.max(discrete_target, 1)[1])
            #
            # # Continuous component
            # continuous_target = final_noise[:, opt.z_dim + opt.disc_dim:]
        ...
        # λI(c, G(c, z)) where I is the approximated mutual information between our prior and the GAN distribution
        # '''
        # ###########################
        netQ.zero_grad()
        input_resv = Variable(input_res)
    ...
    print('[%d/%d] Loss_D: %.4f Loss_G: %.4f, MI_loss: %.4f, c_errG_fake:%.4f' % (epoch, opt.nepoch, D_cost.item(), G_cost.item(), MI_loss.item(), c_errG_fake.item()))
