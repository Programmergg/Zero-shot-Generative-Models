'''
Now its attrs_best performance is unseen=59.43,seen=67.13,h=63.05
FisherGAN
From the abstract:
...
if opt.cuda:
    ...
    LAMBDA = LAMBDA.cuda().requires_grad_()
    RHO = RHO.cuda().requires_grad_()
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        for iter_d in range(opt.critic_iter):
            ...
            X_labels = torch.ones(input_res.shape[0],1).cuda()
            G_labels = torch.zeros(input_res.shape[0],1).cuda()
            ...
            DX_moment_1, DG_moment_1 = criticD_real.mean(), criticD_fake.mean()
            DX_moment_2, DG_moment_2 = (criticD_real**2).mean(), (criticD_fake**2).mean()

            #Compute constraint on second order moments
            OMEGA = 1 - (0.5 * DX_moment_2 + 0.5 * DG_moment_2)

            D_cost = -((DX_moment_1 -DG_moment_1) + LAMBDA * OMEGA - (RHO/2)* (OMEGA**2)) + gradient_penalty
            IMP_ratio = DX_moment_1 - DG_moment_1/0.5 * (DX_moment_2-DG_moment_2)**0.5

            D_cost.backward()
            # Minimize lambda for 'artisinal SGD'
            LAMBDA = LAMBDA + RHO * LAMBDA.grad
            LAMBDA = LAMBDA.detach().cuda().requires_grad_()
    ...
    print("Epoch[%d/%d], G Loss: %.4f, D Loss: %.4f, IPM ratio: %.4f, Lambda: %.4f"% (epoch, opt.nepoch, G_cost.item(), D_cost.item(),IMP_ratio, LAMBDA))
