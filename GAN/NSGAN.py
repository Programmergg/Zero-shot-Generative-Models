'''
Now its attrs_best performance is unseen=54.87,seen=63.87,h=59.03
Non-saturating GAN
...
import torch
import util
from models import MLP_G,MLP_D_2
import classifier
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        Input: the batch of visual features
        Output: D_loss(non-saturing loss for discriminator) -E[log(D(x))]-E[log(1-D(G(z)))]
        '''
        for iter_d in range(opt.critic_iter):
            ...
            D_cost = torch.sum(-torch.mean(torch.log(criticD_real + 1e-8) + torch.log(1 - criticD_fake + 1e-8))) + gradient_penalty
        ...
        (2) Update G network: 
        Input: the batch of visual features
        Output: G_loss(non-saturating loss for how well G(z) fools D)
        -E[log(D(G(z)))]
        '''
        ###########################
        for p in netD.parameters(): # reset requires_grad
        ...
        G_cost = -torch.mean(torch.log(criticG_fake + 1e-8))
