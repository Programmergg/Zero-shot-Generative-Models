'''
Now its attrs_best performance is unseen=54.73,seen=65.37,h=59.58
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        for iter_d in range(opt.critic_iter):
            ...
            D_cost = (0.5 * torch.mean((criticD_real - 1) ** 2)) + (0.5 * torch.mean((criticD_fake - 0) ** 2)) + gradient_penalty
        ...
        G_cost = 0.50 * torch.mean((criticG_fake - 1) ** 2)
