'''
Now its attrs_best performance is unseen=48.74,seen=63.37,h=55.10
...
import torch
import util
from models import MLP_G,MLP_D_3
import classifier
...
G_scheduler = ReduceLROnPlateau(optimizerG, factor=0.50, threshold=0.01, patience=5 * data.ntrain)
D_scheduler = ReduceLROnPlateau(optimizerD, factor=0.50, threshold=0.01, patience=5 * data.ntrain)
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        for iter_d in range(opt.critic_iter):
            ...
            DX_reconst, pred_real= netD(input_resv, input_attv)
            hidden = torch.cat((input_resv, input_attv), 1)
            DX_loss = torch.mean(torch.sum(torch.abs(DX_reconst - hidden), dim=1))
            ...
            DG_reconst, pred_fake = netD(fake.detach(), input_attv)
            hidden2 = torch.cat((fake, input_attv), 1)
            DG_loss = torch.mean(torch.sum(torch.abs(DG_reconst - hidden2), dim=1))

            # gradient penalty
            gradient_penalty = calc_gradient_penalty(netD, input_resv, fake.data,input_attv)

            D_cost = DX_loss - K * DG_loss + gradient_penalty
            D_cost.backward()
            optimizerD.step()
        ...
        DG_reconst, pred_fake2 = netD(fake, input_attv)
        hidden3 = torch.cat((fake, input_attv), 1)

        G_cost = torch.mean(torch.sum(torch.abs(DG_reconst - hidden3), dim = 1))
        ...
        convergence = (DX_loss + torch.abs(0.5 * DX_loss - DG_loss)).item()
        K_update = (K + 1e-3 * (0.5 * DX_loss - DG_loss)).item()
    ...
    print('[%d/%d] Loss_D: %.4f Loss_G: %.4f, c_errG_fake:%.4f, K: %.4f, Convergence Measure: %.4f' % (epoch, opt.nepoch, D_cost.item(), G_cost.item(), c_errG_fake.item(), K, convergence))
