'''
total_variation:its attrs_best performance is unseen=58.35,seen=67.73,h=62.69
forward_kl:its attrs_best performance is unseen=57.73,seen=72.00,h=64.08
reverse_kl:its attrs_best performance is unseen=60.11,seen=67.83,h=63.73
pearson:its attrs_best performance is unseen=57.54,seen=72.56,h=64.18
hellinger:its attrs_best performance is unseen=58.25,seen=69.64,h=63.44
jensen_shannon:its attrs_best performance is unseen=58.76,seen=69.59,h=63.72
f-Divergence GANs
...
parser.add_argument('--methods',default='total_variation',help='choose divergence for GANs')
...
assert opt.methods in ['total_variation','forward_kl','reverse_kl','pearson','hellinger','jensen_shannon'], 'Invalid divergence'
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        for iter_d in range(opt.critic_iter):
            ...
            if opt.methods == 'total_variation':
                D_cost = -(torch.mean(0.5 * torch.tanh(criticD_real)) - torch.mean(0.5 * torch.tanh(criticD_fake))) + gradient_penalty
            elif opt.methods == 'forward_kl':
                D_cost = -(torch.mean(criticD_real) - torch.mean(torch.exp(criticD_fake - 1))) + gradient_penalty
            elif opt.methods == 'reverse_kl':
                D_cost = -(torch.mean(-torch.exp(criticD_real)) - torch.mean(-1 - criticD_fake)) + gradient_penalty
            elif opt.methods == 'pearson':
                D_cost = -(torch.mean(criticD_real) - torch.mean(0.25 * criticD_fake**2 + criticD_fake)) + gradient_penalty
            elif opt.methods == 'hellinger':
                D_cost = -(torch.mean(1 - torch.exp(criticD_real)) - torch.mean((1 - torch.exp(criticD_fake))/(torch.exp(criticD_fake)))) + gradient_penalty
            elif opt.methods == 'jensen_shannon':
                D_cost = -(torch.mean(torch.tensor(2.) - (1 + torch.exp(-criticD_real))) - torch.mean(-(torch.tensor(2.) - torch.exp(criticD_fake)))) + gradient_penalty
        ...
        if opt.methods == 'total_variation':
            G_cost = -torch.mean(0.5 * torch.tanh(criticG_fake))
        elif opt.methods == 'forward_kl':
            G_cost = -torch.mean(torch.exp(criticG_fake - 1))
        elif opt.methods == 'reverse_kl':
            G_cost = -torch.mean(-1 - criticG_fake)
        elif opt.methods == 'pearson':
            G_cost = -torch.mean(0.25 * criticG_fake**2 + criticG_fake)
        elif opt.methods == 'hellinger':
            G_cost = -torch.mean((1 - torch.exp(criticG_fake))/(torch.exp(criticG_fake)))
        elif opt.methods == 'jensen_shannon':
            G_cost = -torch.mean(-(torch.tensor(2.) - torch.exp(criticG_fake)))
