'''Now its attrs_best performance is unseen=0.4535, seen=0.3550, h=0.3983
'''
import argparse
import random
import torch
...
parser.add_argument('--dataset', default='SUN', help='AWA,AWA2,APY,CUB,SUN')
...
parser.add_argument('--REG_W_LAMBDA',type=float,default=0.0001,help = 'the regularization for generator')
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        errG = G_cost
        # errG = G_cost + opt.cls_weight * c_errG_fake
        errG.backward()
        optimizerG.step()
