'''
Now its attrs_best performance is unseen=46.73,seen=59.32,h=52.28
Relativistic GAN
...
import torch
import util
from models import MLP_D_2,MLP_G
import classifier
...
for epoch in range(opt.nepoch):
    for i in range(0, data.ntrain, opt.batch_size):
        ...
        for iter_d in range(opt.critic_iter):
            ...
            D_cost = -torch.mean(torch.log(torch.sigmoid(criticD_real-criticD_fake.mean()) + 1e-8) + torch.log(torch.sigmoid(1 - criticD_fake) + 1e-8)) / 2 + gradient_penalty
